...
public class BridgeObstacle : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider collision)
    {
        if (collision.CompareTag("Player") || collision.CompareTag("Enemy"))
        {
            ...
            {
                if (collision.GetComponentInParent<Enemy>().size > minimumSize)
                {
                    rb.mass = 250 - 249 * (collision.GetComponentInParent<Enemy>().size - minimumSize) / minimumSize;
                    js = hj.spring;
                    js.damper = 2500 - 2499 * (collision.GetComponentInParent<Enemy>().size - minimumSize) / minimumSize;
                    hj.spring = js;
                }
                else
                {
            ...
            if (!open)
            {
                ...
                //if (collision.transform.GetComponent<Player>().size > minimumSize)
                {
                    ...
                    {
                        BalancePiece.GetComponent<Collider>().enabled = false;
                        open = true;
                    }
                }
            }
